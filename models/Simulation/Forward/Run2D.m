function model = Run2D(model, visualization)
%
% DESCRIPTION: runs the simulation for defined schedule
%
% SYNOPSIS:
%   model = Run2D(model, visualization)
%
% PARAMETERS:
...
for i = 1 : height(scheduleTable)
    ...
    model.dynamic  = dynamic;
    dynamic = UpdateDynamicParams2D(model); % update dynamic parameters
    model.dynamic = dynamic;
    if visualization
...
%---------------------------------------
model = save_for_HM_2D(model,state0);
%---------------------------------------
% output results
%---------------------------------------
if visualization
