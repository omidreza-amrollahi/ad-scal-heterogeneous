...
schedule = struct;
experiment = struct;
experiment.rock.perm_heterogeneity.include = false;
simulation = struct;
observation = struct;
output = [];
...
while ~feof(fid)
    ...
    if ~feof(fid)
        ...
        switch kw
            ...
            case 'PERMEABILITY_HETEROGENEITY'
                value = str2double(splitted_lin{2});
                unit = string(splitted_lin{3});
                rock.perm_heterogeneity.include = true;
                rock.perm_heterogeneity.inputValue = value;
                rock.perm_heterogeneity.inputUnit = string(unit);
                rock.perm_heterogeneity.value = value * Convert(unit);
            case 'INITIALWATER'
                value = str2double(splitted_lin{2});
            ...
            case 'POROSITY_PROFILE'
                rock.poro.include = true;
                rock.poro.fullFile = abs_path(splitted_lin{2}, path_to_settings);
            ...
            case 'PC'
                ...
                pc.scaling_factor.value = 1;
                satfun.pc = pc;
            case 'PC_SCALING_FACTOR'
                pc.scaling_factor.value = str2double(splitted_lin{2});
                satfun.pc = pc;
